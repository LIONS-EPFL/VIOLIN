...
class VideoGenerator:
    ...
    def _inference(self, inp: str, out: str):
        ...
        for img_path in tqdm(sorted(glob.glob(os.path.join(inp, "*.jpg")))):
            ...
            w, h = (
                img.shape[1] - img.shape[1] % 16,
                img.shape[2] - img.shape[2] % 16,
            )
    ...
    def __load_model(self):
        ...
        if args.arch in violin.__dict__.keys():
            model = violin.__dict__[args.arch](
                num_classes = 0,
                patch_size = self.args.patch_size,

                cls_tok = self.args.cls_tok,
                pos_emb = self.args.pos_emb,
                scale = self.args.scale,
                initialize = self.args.initialize,
                curve_list = self.args.curves,
                mask = self.args.mask,
        ...
        if os.path.isfile(self.args.pretrained_weights): 
            state_dict = torch.load(self.args.pretrained_weights, map_location="cpu",   weights_only=False )
...
def parse_args():
    ...
    parser.add_argument(
        "--arch",
        default="violin_small",
        type=str,
        choices=['violin_tiny', 'violin_small', 'violin_base',
